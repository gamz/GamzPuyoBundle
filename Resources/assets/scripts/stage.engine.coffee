...
class puyo.stage.MatchingGroup extends puyo.stage.Group
...
class puyo.stage.ControlGroup
    @NORTH: 0
    @EAST:  1
    @SOUTH: 2
    @WEST:  3
    constructor:            -> @moves =
        left:  new puyo.stage.ControlLeft()
        right: new puyo.stage.ControlRight()
        down:  new puyo.stage.ControlDown()
        turn:  new puyo.stage.ControlTurn()
    bind: (@c1, @c2)        ->
    unbind:                 -> [@c1, @c2] = [null, null]
    invert:                 -> [@c1, @c2] = [@c2, @c1]
    move: (move)            -> if @moves[move].test @ then (@moves[move].apply @ ; true) else false

class puyo.stage.ControlTrans
    test: (group, dx, dy)   -> [a = group.c1.test(dx, dy, group.c2), b = group.c2.test(dx, dy, group.c1)] ; a and b

class puyo.stage.ControlDown extends puyo.stage.ControlTrans
    test: (group)           -> switch group.orient
        when puyo.stage.ControlGroup.SOUTH then group.c2.test 0, 1
        when puyo.stage.ControlGroup.NORTH then group.c1.test 0, 1
        else super(group, 0, 1)
    apply: (group)          -> switch group.orient
        when puyo.stage.ControlGroup.SOUTH then group.c2.move 0, 1 ; group.c1.move 0, 1
        else                        group.c1.move 0, 1 ; group.c2.move 0, 1

class puyo.stage.ControlLeft extends puyo.stage.ControlTrans
    test: (group)           -> switch group.orient
        when puyo.stage.ControlGroup.WEST then group.c2.test -1, 0
        when puyo.stage.ControlGroup.EAST then group.c1.test -1, 0
        else super(group, -1, 0)
    apply: (group)          -> switch group.orient
        when puyo.stage.ControlGroup.WEST then group.c2.move -1, 0 ; group.c1.move -1, 0
        else                       group.c1.move -1, 0 ; group.c2.move -1, 0

class puyo.stage.ControlRight extends puyo.stage.ControlTrans
    test: (group)           -> switch group.orient
        when puyo.stage.ControlGroup.EAST then group.c2.test 1, 0
        when puyo.stage.ControlGroup.WEST then group.c1.test 1, 0
        else super(group, 1, 0)
    apply: (group)          -> switch group.orient
        when puyo.stage.ControlGroup.EAST then group.c2.move 1, 0 ; group.c1.move 1, 0
        else                       group.c1.move 1, 0 ; group.c2.move 1, 0

class puyo.stage.ControlTurn
    delta: (group)          -> switch group.orient
        when puyo.stage.ControlGroup.NORTH then [ 1,  1]
        when puyo.stage.ControlGroup.EAST  then [-1,  1]
        when puyo.stage.ControlGroup.SOUTH then [-1, -1]
        when puyo.stage.ControlGroup.WEST  then [ 1, -1]
